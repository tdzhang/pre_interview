...
* 请发布小程序，TODO(XXXXXXX)
* 将整个工程文件的压缩包作为邮件附件，请注意提供部署服务器的说明文档，TODO(XXXXXXX)
* 注册github账号，把代码上传到github，直接发送github项目链接给我们，TODO(XXXXXXX)
...
* 调用接口： http://47.75.66.0:80/interview_api/price_time_series
* 调用方法：POST
* 参数格式：json
...
curl --header "Content-Type: application/json" \
  	 --request POST \
  	 --data '{"type":"all"}' \
  	 http://47.75.66.0:80/interview_api/price_time_series
```
* 输出：为JSON格式的数据，例子如下
```
返回对应时间段的数据（从当前时刻起）
...
* 调用接口： http://47.75.66.0:80/interview_api/dynamic_table1
* 调用方法：POST
* 参数格式：json
* 输入参数：无输入参数，例子如下
```
...
curl --header "Content-Type: application/json" \
  	 --request POST \
  	 --data '{}' \
  	 http://47.75.66.0:80/interview_api/dynamic_table1
```
...
* 调用接口： http://47.75.66.0:80/interview_api/dynamic_table2
* 调用方法：POST
* 参数格式：json
* 输入参数：无输入参数，例子如下
```
...
curl --header "Content-Type: application/json" \
  	 --request POST \
  	 --data '{}' \
  	 http://47.75.66.0:80/interview_api/dynamic_table2
```
* 输出：返回随时间变化动态买卖价格（总计需要前端完成计算），例子如下
```
{
...
* 调用接口： http://47.75.66.0:80/interview_api/server_status
* 调用方法：POST
* 参数格式：json
* 输入参数：无输入参数，例子如下
```
...
curl --header "Content-Type: application/json" \
  	 --request POST \
  	 --data '{}' \
  	 http://47.75.66.0:80/interview_api/server_status
```
* 输出：，例子如下
```
{
...
加密API如下：
* 调用接口： http://47.75.66.0:80/interview_api/encrypt
* 调用方法：POST
* 参数格式：json
...
curl --header "Content-Type: application/json" \
  	 ...
  	 http://47.75.66.0:80/interview_api/encrypt
```
* 输出：返回加密后的字符串，例子如下
```
{
...
解密API如下：
* 调用接口： http://47.75.66.0:80/interview_api/decrypt
* 调用方法：POST
* 参数格式：json
...
curl --header "Content-Type: application/json" \
  	 ...
  	 http://47.75.66.0:80/interview_api/decrypt
```
* 输出：返回解密后的字符串，例子如下
```
{
